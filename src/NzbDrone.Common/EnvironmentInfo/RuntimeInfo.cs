    ...
    public class RuntimeInfo : IRuntimeInfo
    {
        ...
        public RuntimeInfo(IServiceProvider serviceProvider, Logger logger)
        {
            ...
            if (entry != null)
            {
                ...
                IsWindowsTray = OsInfo.IsWindows && entry.ManifestModule.Name == $"{ProcessProvider.GAMEARR_PROCESS_NAME}.exe";
        ...
        public bool IsTray
        {
            get
            {
                if (OsInfo.IsWindows)
                {
                    return IsUserInteractive && Process.GetCurrentProcess().ProcessName.Equals(ProcessProvider.GAMEARR_PROCESS_NAME, StringComparison.InvariantCultureIgnoreCase);
                }
